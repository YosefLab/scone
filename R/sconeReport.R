...
subsample_cells_with_min_bio <- function(scone_object, seed = 100, verbose= FALSE){
  ...
  for(bio in bio_uniq){
    ...
    cells_with_bio <- scone_object[,(bios == bio)]
  ...
  inter@metadata$supcolnames <- c(inter@metadata$supcolnames, colnames(scone_object))
  if(verbose){
    print(paste("Initial Size", toString(ncol(scone_object)), "Cells"))
