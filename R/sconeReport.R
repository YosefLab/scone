...
#'   biological phenomenon of interest. Default empty character.
#'   
#' @importFrom shiny fluidPage downloadLink br titlePanel tableOutput helpText downloadHandler column h5 h6 updateSelectInput plotOutput tabsetPanel selectInput renderTable p sidebarLayout sidebarPanel shinyApp mainPanel observeEvent renderPlot reactive fluidRow sliderInput tabPanel
#' @import visNetwork
#' @importFrom DT dataTableProxy dataTableOutput renderDataTable selectRows formatSignif
#' @import plotly
...
sconeReport = function(scone_res, qc, 
  ...
  all_nodes = unique(gsub("^,*|,*$","",gsub("\\,+",",",gsub("none|no_uv|no_bio|no_batch","",all_nodes))))
  
  # Order by decreasing number of operations (root at the end)
  all_nodes = all_nodes[order(paste0(gsub(".+",",",all_nodes),gsub("[^,]*","",all_nodes)),decreasing = TRUE)]
  
  # Collapse no-op names for all leaves
  leaves = gsub("^,*|,*$","",gsub("\\,+",",",gsub("none|no_uv|no_bio|no_batch","",leaves)))
    ...
    sidebarLayout(
      sidebarPanel(
        selectInput("norm_code", label = "Normalization",
                    choices = structure(as.list(rownames(scone_res$params)),names = rownames(scone_res$params)),
      ...
      mainPanel(tabsetPanel(type = "tabs",
                            ...
                            tabPanel("PCA",
                                     ...
                                     selectInput("gene_set", label = "Gene selection",
                                                 ...
                                                 selected = "neg"),
                                     h6("2D (Select Genes)"),
                                     p("Visualize cells by PCs of control gene sets."),
                            ...
                            tabPanel("Silhouette",
                                     ...
                                     selectInput("cat1", label = "Row Class",
                                                 choices = list("Batch"="batch", "Biological Condition"="bio", "PAM Cluster"="clust"),
                                                 selected = "batch"),
                                     selectInput("cat2", label = "Column Class",
                                                 choices = list("Batch"="batch", "Biological Condition"="bio", "PAM Cluster"="clust"),
                                                 selected = "bio"),
                                     tableOutput("cat_tab")
                            ),
                            tabPanel("Control Genes",
                                     br(),
                                     p("Heatmap of control genes, colored by all three categories."),
                                     br(),
                                     p("Positive controls:"),
                            ...
                            tabPanel("Stratified PCA",
                                     br(),
                                     p("Violin plots showing conditional distributions of selected Principal Component."),
                                     br(),
                                     sliderInput("pcsel", label = "Selected Principal Component",
                                                 min=1, max=min(length(batch)-1,10),
                                                 value = 1),
                                     h6("All Genes:"),
                            ...
                            tabPanel("Relative Log-Expression",
                                     br(),
                                     p("Relative Log-Expression Plot for top 100 Most Variable Genes."),
  ...
  server <- function(input, output, session) {
    ...
    output$norm_net <- renderVisNetwork({
      ...
      if(any(!all_nodes  %in% leaves)){
        visNetwork(nodes, edges,width = "100%",main = "Tree of Methods") %>% 
          ...
          visOptions(nodesIdSelection = list(enabled = TRUE, values = nodes$id[nodes$group == "Loaded"], selected = nodes$id[nodes$title == rownames(scone_res$params)[1]])) %>%
    ...
    output$tbl_score <- DT::renderDataTable({
      DT::datatable(scone_res$scores,
                    ...
                    selection = list( mode = 'single',
                                      target = 'row',
                                      selected = c(1)),
                    options=list(columnDefs = list(list(visible=FALSE, targets=1:(ncol(scone_res$scores)-1))),
                                 dom = 'Bfrtip', 
                                 buttons = list(list(extend = 'colvis', columns = c(1:(ncol(scone_res$scores)-1)))))) %>%
    ...
    output$plot_scree <- renderPlot({
      plot(pc_obj_base()$sdev[1:min(input$dim*2,length(batch)-1)]^2/sum(pc_obj_base()$sdev^2), typ = "l", ylab = "Proportion of Variance", xlab = "PC Index", log = "y")
      ...
      if(length(negcon) > 0){
        ...
        if(length(unique(strat_col())) > 1){
          ...
          aheatmap(normle()[negcon,], Colv = silo(), 
                   annCol = list(batch = batch, bio = bio, pam =  as.factor(pam_obj()$clust)),
    ...
    output$downloadData <- downloadHandler(
      ...
      content = function(con) {
        datt = cbind(as.character(bio),as.character(batch),as.character(pam_obj()$clust))
