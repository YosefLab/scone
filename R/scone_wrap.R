...
#'     the value of the "norm_impute" argument. Default 1000.
#'  
...
scone_easybake <- function(expr, qc,
  ...
  if(is.null(negcon) && fnr_maxiter > 0){stop("negcon must be specified if fnr_maxiter > 0")}
  ...
  if(fnr_maxiter > 0) {
    ...
    ## fnr_maxiter <= 0 (should be 0, check for <= 0 for robustness) -- user doesn't want to do fnr estimation
    if(verbose > 0) {
