...
load("/data/yosef/users/mbcole/SOX2/E3_gf4020.rda")
...
scone_out = SCONE(e = gf4020,factor_free_only = F,design = c("nested"), nested_model = c("fixed"),
                  condition = expt,
                  batch = c1_run_id,
                  qual = PPQual(QCdataE3),
                  dim_UV = 5,
                  is_HK = rownames(gf4020) %in% as.matrix(HKlistE1),
                  is_DE = rownames(gf4020) %in% as.matrix(de),
                  is_CC = toupper(rownames(gf4020)) %in% as.matrix(cc$V2),
                  out_dir = "/data/yosef/users/mbcole/SOX2/SCONE_out_4020")