...
test_that("Test with no real method (only identity)", {
  ...
  params <- scone(e, imputation=list(impute_null,impute_null),
                  scaling=list(identity, identity, identity), k_ruv=5,
  ...
  res <- scone(e, imputation=list(impute_null,impute_null),
               scaling=list(identity, identity, identity), k_ruv=5,
...
test_that("Test imputation and scaling", {
  ...
  params <- scone(e, imputation=list(none=impute_null, zinb=impute_zinb),
                  scaling=list(none=identity, uq=UQ_FN, deseq=DESEQ_FN),
  ...
  # evaluation
  system.time(res <- scone(e, imputation=list(none=impute_null, zinb=impute_zinb),
                           scaling=list(none=identity, uq=UQ_FN, deseq=DESEQ_FN),
  ...
  res2 <- scone(e, imputation=list(none=impute_null, zinb=impute_zinb),
               scaling=list(none=identity, uq=UQ_FN, deseq=DESEQ_FN),
               k_ruv=5, k_qc=2, ruv_negcon=as.character(1:10),
               qc=qc_mat, adjust_bio="yes", bio=bio,
               adjust_batch="yes", batch=batch, run=TRUE,
...
test_that("scone works with only one normalization",{
  ...
  res <- scone(e, imputation=list(none=impute_null),
...
test_that("conditional PAM",{
  ...
  expect_error(res <- scone(e, imputation=list(none=impute_null, zinb=impute_zinb),
               scaling=list(none=identity, uq=UQ_FN, deseq=DESEQ_FN),
...
test_that("if bio=no bio is ignored", {
  ...
  res1 <- scone(e, imputation=impute_null, scaling=identity, k_ruv=0, k_qc=0,
               adjust_bio = "no", bio=gl(2, 5), eval_kclust = 3)

  res2 <- scone(e, imputation=impute_null, scaling=identity, k_ruv=0, k_qc=0,
...
test_that("batch and bio can be confounded if at least one of adjust_bio or adjust_batch is no", {
  ...
  expect_warning(scone(e, imputation=impute_null, scaling=identity, k_ruv=0, k_qc=0,
