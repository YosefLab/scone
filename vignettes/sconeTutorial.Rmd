...
The `rsem_counts` assay contains expected gene-level read counts via RSEM [@li2011] quantification of 130 single-cell libraries aligned to the hg38 RefSeq transcriptome. The data object also contains library transcriptome alignment metrics obtained from [Picard](http://broadinstitute.github.io/picard/) and other basic tools.

```{r showqc, eval=TRUE}

## ----- List all QC fields -----

metadata(fluidigm)$which_qc # List all qc fields (accessible via colData())
...
Not only does `scone` normalize expression data, but it also provides a framework for evaluating the performance of normalization workflows. In order to run the main `scone` function, we will need to decide which workflows (normalizations) we will want to compare. Prior to running `scone` we will want to define several input arguments.
...
## ----- Imputation Function Arguments, accessible by functions in imputation list argument ----- 
...
my_scone <- sconeExperiment(expr, # Primary expression data
                qc=ppq, bio = bio, # QC metrics and sample metadata
                negcon_ruv = rownames(expr) %in% negcon, # Negative controls for both normalization and evaluation
                poscon = rownames(expr) %in% poscon # Positive controls for evaluation
)
...
The main `scone` function arguments allow for a lot of flexibility, but a user may choose to run very specific combinations of modules. For this purpose, `scone` can be run in `run=FALSE` mode, generating a list of workflows to be performed.
...
Some scaling methods, such as scaling by gene detection rate (`EFF_FN()`), will not make sense within the context of imputed data, as imputation replaces zeroes with non-zero values. We can remove unnessary rows from `params` to produce a data frame of meaningful normalization workflows.

```{r scone_params_filt}

is_screened = ((get_params(my_scone)$imputation_method == "expect") & (get_params(my_scone)$scaling_method %in% c("none","deseqp","uqp","fqp","eff")))
...
Now that we have selected our workflows, we can run `scone` in `run=TRUE` mode. This mode relies on a few additional arguments, including an optional `params` argument for passing results from a `run=FALSE` run. In order to understand these arguments, we must first understand the 8 metrics used to evaluate each normalization. The first 6 metrics rely on a reduction of the normalized data down to 3 dimensions via PCA (default). Each metric is taken to have a positive (higher is better) or negative (lower is better) signature.
...
* EXP_QC_COR: Removal of Alignment Artifacts. Maximum squared Spearman correlation between first 3 expression PCs and first `k_qc` QPCs. Negative signature.
* EXP_UV_COR: Removal of Expression Artifacts. Maximum squared Spearman correlation between first 3 expression PCs and first 3 PCs of the negative control (specified by `eval_negcon` or `ruv_negcon` by default) sub-matrix of the original (raw) data. Negative signature.
* EXP_WV_COR: Preservation of Biological Variance. Maximum squared Spearman correlation between first 3 expression PCs and first 3 PCs of the positive control (specified by `eval_poscon`) sub-matrix of the original (raw) data. Positive signature.
...
# Look at normalized data from top method
head(get_normalized(my_scone,method = rownames(get_params(my_scone))[1]))
...
bp_obj = biplot_colored(pc_obj,y = -get_score_ranks(my_scone),expand = .6)
...
scone_methods = c(rownames(get_params(my_scone))[1:12],"none,none,no_uv,no_bio,no_batch")
scone_res = list()
scone_res$normalized_data = lapply(as.list(scone_methods), 
      FUN = function(x){get_normalized(my_scone,method = x,log=TRUE)})
names(scone_res$normalized_data) = scone_methods
scone_res$params = get_params(my_scone)[scone_methods,]
scone_res$scores = cbind(get_scores(my_scone),get_score_ranks(my_scone))[scone_methods,]
colnames(scone_res$scores)[ncol(scone_res$scores)] = "mean_score_rank"

sconeReport(scone_res = scone_res,
            qc = ppq,
            bio = bio,
