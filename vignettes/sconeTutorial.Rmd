...
`scone` arguments allow for a lot of flexibility, but a user may choose to run very specific combinations of modules. For this purpose, `scone` can be run in `run=FALSE` mode, returning a data frame of workflows to be performed.
...
params <- scone(expr, 
                ...
                scaling=scaling, 
                qc=ppq, bio = bio,
                ruv_negcon = negcon, # Negative controls for RUVg normalization and evaluation
                
                k_qc=3, k_ruv = 3, # Parameter Arguments
                adjust_bio="no", 
                run=FALSE)

head(params) # Visualize output params object
